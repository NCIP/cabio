...
public class SODUtils{
  ...
  private SODUtils(){
    try{
      SODFactory fact = 
	(SODFactory)Class.forName( AppConfig.getInstance().getSODFactoryClassname() ).newInstance();
      _sod = fact.getSOD();
    }catch( Exception ex ){
  ...
  public SearchableObject getSearchableObject( SearchableObjectsDescription sod, 
						      String objClassname ){
    if( objClassname == null ){
      throw new IllegalArgumentException( "SearchableObject classname is null" );
    }
	
    boolean usingShortName = objClassname.indexOf( "." ) == -1;
    if( objClassname.endsWith( "Impl" ) ){
      objClassname = objClassname.substring( 0, objClassname.indexOf( "Impl" ) );
    }
  ...
  public List getAssociatedObjects( SearchableObjectsDescription sod, SearchableObject so ){
	...
	try{
	  /*
	    SODFactory fact = 
		(SODFactory)Class.forName( "gov.nih.nci.caBIO.ui.sod.SODFactoryImpl" ).newInstance();
  ...
  public Association getAssociationWithRole( String objectName, String role ){
    ...
    if( so == null ){
      throw new RuntimeException( "No SearchableObject found for " + objectName );
    }
    return getAssociationWithRole( so, role );
  }

  public Association getAssociationWithRole( SearchableObject so, String roleName ){
    if( so == null ){
  ...
  public String getBeanNameFromPath( String path ){
	...
	while( st.hasMoreTokens() ){
	  ...
	  nextAssoc = getAssociationWithRole( prev.getClassname(), role );
	  if( nextAssoc == null ){
	    throw new RuntimeException( "Assocation not found for " +
					prev.getClassname() +
					" and " + role );
