...
public class ExcelParser implements TableParser{
  ...
  public ExcelParser( Integer sheetNum, Integer headerRowNum, Integer dataStartRowNum ){
    _sheetNum = sheetNum;
    ...
    throws IndexOutOfBoundsException, IOException
  {
    ...
    Spreadsheet sheet = parser.parseExcelSpreadsheet( in, _sheetNum.intValue() );
	...
	headers = new Cell[0];
      }else if( _headerRowNum.intValue() > -1 && _headerRowNum.intValue() < numRows ){
	headers = rows[_headerRowNum.intValue()].getCells();
      }else{
	throw new IndexOutOfBoundsException( "Header row number " + 
					     (_headerRowNum.intValue() + 1) + 
					     " is out of range" );
      }
      if( _dataStartRowNum == null ){
	_dataStartRowNum = new Integer( 0 );
      }else if( _dataStartRowNum.intValue() < 0 || _dataStartRowNum.intValue() >= numRows ){
	...
	try{
	  ...
	  throw new RuntimeException( "Error getting header value at [" + 
				      _headerRowNum + "," + i + "]: " + ex.getMessage() );
	}
	...
	for( int col = 0; col < numCols; col++ ){
	  if( r != null ){
	    try{
	      if( col < cells.length ){
		table.setStringValueAt( (String)cells[col].getCellValue(),
					row - offset, col );
	      }else{
		...
		table.setStringValueAt( "", row - offset, col );
	      ...
	      throw new RuntimeException( "Error setting string value at [" + 
					  (row - offset) + "," + col + "]: " + ex.getMessage() );
	    }
	  }else{//row is empty
	    table.setStringValueAt( "", row - offset, col );
	  }
