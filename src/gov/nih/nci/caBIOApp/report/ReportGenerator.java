...
public class ReportGenerator {
  ...
  private void init(){
    try{
      ...
      throw new RuntimeException( "Couldn't initialize matrix size: " + ex.getMessage() );
    }
  }
    ...
    throws Exception
  {
    try{
      ...
      */
      ...
      GridSearchCriteria sc = new GridSearchCriteria( selectionTree, batchSearch );
	...
	if( rowIndices.size() == 0 ){//means we are NOT merging
	  for( int j = 0; j < rows.length; j++ ){
		...
		MessageLog.printInfo( " adding " + val + " cell to [" + j + "," + theCol + "]" );
		tableRow[theCol] = val;
	      }else{
		MessageLog.printInfo( "cell for " + csp + " is null." );
	      }
	    }
	  ...
	  if( rowIndex == null ){ MessageLog.printInfo( "ROW INDEX IS NULL" ); }
	  ...
	  for( Iterator rit = rowsToDup.iterator(); rit.hasNext(); ){
	    ...
	    if( gridRows != null && gridRows.length > 0 ){
	      MessageLog.printInfo( gridRows.length + " gridRows for mapping " + i );
		...
		if( gridRow != null ){
		  for( ListIterator j = colSpecs.listIterator(); j.hasNext(); ){
			...
			csp = basePath + "." + csp;
		      ...
		      if( gridCell != null ){
			rowCopy.set( j.previousIndex(), gridCell.getObject() );
		      }
		    }
  ...
  private SelectionNode buildSelectionTree( gov.nih.nci.common.search.SearchCriteria criteria, List joinObjs, List colSpecs )
    throws Exception
  {
    ...
    SelectionNode tree = new SelectionNodeImpl( mainObjShortName, criteria, mainObjAtts );
    ...
    MessageLog.printInfo( "ReportGenerator.buildSelectionTree(): mergeObjName = " + mergeObjName );
    int idx = 0;
	...
	filter = criteria.getSearchCriteria( "gov.nih.nci.caBIO.bean." + objName + "SearchCriteria" );
	if( filter != null ){
	  MessageLog.printInfo( "...found merge object" );
	}
      }else{
	filter = (gov.nih.nci.common.search.SearchCriteria)Class.forName( "gov.nih.nci.caBIO.bean." + 
				       objName + "SearchCriteria" ).newInstance();
      }
    ...
    throws Exception
  {
	...
	batch[i.previousIndex()] = new SearchCriteriaMapping( idx, idx.getCriteria() );
      }
    }
    return batch;
  }
  ...
  private List orderNodes( SelectionNode tree, List colSpecs ){
	...
	objName = objName.substring( idx + 1 );
      ...
      for( Enumeration enum = tree.preorderEnumeration(); enum.hasMoreElements(); ){
	SelectionNode node = (SelectionNode)enum.nextElement();
