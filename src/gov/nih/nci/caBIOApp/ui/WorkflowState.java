...
public class WorkflowState implements Serializable{
	...
	if( idx > 0 ){
	    throw new IllegalArgumentException( "action index must be negative or zero" );
	}
	String a = null;
	if( !_actions.isEmpty() ){
	    try{
  ...
  public static WorkflowState getState( HttpServletRequest request, ActionMapping mapping ){

    HttpSession session = request.getSession();
    WorkflowState state = 
      (WorkflowState)session.getAttribute( UIConstants.WORKFLOW_STATE_KEY );
    if( state == null ){
      if( _debug ){ MessageLog.printInfo( "WorkflowState: creating new state" ); }
      state = new WorkflowState();
      session.setAttribute( UIConstants.WORKFLOW_STATE_KEY, state );
    }
    String thisAction = mapping.getPath();
    if( _debug ){
      MessageLog.printInfo( "WorkflowState: mapping.getPath() = " + mapping.getPath() +
			    ", state.getAction() = " + state.getAction() );
    }
    if( thisAction.equals( state.getAction() ) ){
      if( _debug ){ MessageLog.printInfo( "WorkflowState: within same action..." ); }
      if( state.getPopulate() ){
	if( _debug ){ MessageLog.printInfo( "WorkflowState: populating..." ); }
	...
	state.setLastStep( request.getParameter( UIConstants.LAST_STEP_KEY ) );
      ...
      if( _debug ){ MessageLog.printInfo( "WorkflowState: resetting..." ); }
      state.reset();
