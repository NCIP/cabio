...
public class CaBIOPagerDataSource2
  implements PagerDataSource
{
  ...
  public void setEventHandler( String s ){ _eventHandler = s; }
...
// 			", numItems = " + numItems );
	...
	for( ListIterator j = assocs.listIterator(); j.hasNext(); ){
	  ...
	  vals.add( 
	    "<a href=\"javascript:" + _eventHandler + "('" + obj.getClass().getName() +
  ...
  protected void initHeaders(){
    ...
    for( Iterator it = labelProps.iterator(); it.hasNext(); ){
      ...
      if( att == null ){
	throw new RuntimeException( "CaBIOPagerDataSource.init(): couldn't find attribute " + 
				    propName +
				    " on " + so.getClassname() );
      }
      h.add( att.getLabel() );
