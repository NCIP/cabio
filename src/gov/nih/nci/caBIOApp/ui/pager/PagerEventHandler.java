...
public class PagerEventHandler extends HttpServlet{
  ...
  /**
   * The session scope key under which the selection of PagerItems elements will be maintained.
   */
  private String _selectedItemsKeyName = null;

  /**
   * The request scope key under which the selection mode may initially be found.
   * Valid values are "true" or "false", defaults to "true".
   */
  private String _allowSelectionKeyName = null;

  /**
   * The request scope key under which the default display size will initially be found.
   * Defaults to 25.
   */
  private String _defaultDisplaySizeKeyName = null;

  /**
   * The request scope key under which the classname of the PagerBean may be found.
   */
    ...
    throws ServletException, IOException
  {
    ...
    if( dataSourceKeyName == null ){ dataSourceKeyName = _dataSourceKeyName; }
    String pagerBeanKeyName = request.getParameter( _pagerBeanKeyNameKeyName );
    if( pagerBeanKeyName == null ){ pagerBeanKeyName = _pagerBeanKeyName; }
    String selectedItemsKeyName = request.getParameter( _selectedItemsKeyNameKeyName );
    if( selectedItemsKeyName == null ){ selectedItemsKeyName = _selectedItemsKeyName; }


    PagerBean bean = null;
    MessageLog.printInfo( "PagerEventHandler.doGet(): looking for datasource under " + dataSourceKeyName );
    ...
    if( dataSource != null ){
      ...
      MessageLog.printInfo( "PageEventHandler: putting pagerBean in session under " + pagerBeanKeyName );
      ...
      if( bean == null ){
	throw new ServletException( "No pager bean found under " + pagerBeanKeyName );
      }
  ...
  private PagerBean initBean( PagerDataSource dataSource, HttpServletRequest request )
    throws ServletException
  {
	...
	bean.setDefaultDisplaySize( Integer.parseInt( s ) );
      ...
      throw new ServletException( "Error setting default display size.", ex );
    }
    return bean;
  }
  ...
  private void forward( HttpServletRequest request, HttpServletResponse response, String forward )
    throws ServletException, IOException
  {
    ...
    if( dispatcher == null ){
      throw new ServletException( "Couldn't get dispatcher for: " + forward );
    }
