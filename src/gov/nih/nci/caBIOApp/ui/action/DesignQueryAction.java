...
public class DesignQueryAction extends Action{
    ...
    throws IOException, ServletException
  {
    ...
    MessageLog.printInfo( "DesignQueryAction.perform(): nextStep = " + nextStep +
			  ", lastStep = " + lastStep );
	...
	if( form.getCallingSubAction() ){
	  ...
	  if( table != null && colStr != null ){
	    ...
	    try{
	      ...
	      throw new ServletException( colStr + "is not a valid column number"  );
	    }
	    ActionErrors errors = form.updateCriterion( table, colNum );
	    if( errors.size() > 0 ){
    ...
    throws ServletException
  {
    ...
    if( design == null ){
      throw new ServletException( "QueryDesign not found under " + _queryDesignKeyName );
    }
