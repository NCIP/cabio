...
import gov.nih.nci.caBIO.bean.OntologySearchCriteria;
...
public class QueryDesign{
    ...
    throws InvalidCriterionException
  {
    ...
    if( _so == null ){
      throw new InvalidCriterionException( "searchable object not found for: " + objectName );
    }
    ...
    throws InvalidCriterionException
  {
    ...
    CriterionValue val = 
      (CriterionValue)((CriterionValue)node.getUserObject()).clone();
    MessageLog.printInfo( "selectCriterion, returning val.getId() = " + val.getId() +
  ...
  public void updateCriterion( CriterionValue newVal, Table datasource, int sourceColNum )
    throws InvalidCriterionException
  {

    SearchCriteriaNode node = findNode( newVal.getId() );
    if( node == null ){
      throw new InvalidCriterionException( "Couldn't find criterion with id: " + 
					   newVal.getId() );
    }
    ...
    MessageLog.printInfo( "QueryDesign.updateCriterion(newVal,datasource,sourceColNum): " + sb.toString() );
    ...
    throws InvalidCriterionException
  {
    ...
    SearchCriteriaNode theParent = 
      (SearchCriteriaNode)node.getParent();
    if( theParent == null ){
  ...
  public void setMergeCriterion( String id )
    throws InvalidCriterionException
  {
    SearchCriteriaNode node = findNode( id );
    if( node == null ){
    ...
    throws InvalidCriterionException
  {
    ...
    do{
      MessageLog.printInfo( "QueryDesign.createObjectNode(), trying to insert: " + currId );
      String beanName = _sod.getBeanNameFromPath( currId );
      SearchableObject so = _sod.getSearchableObject( beanName );
      if( so == null ){
	throw new InvalidCriterionException( "QueryDesign.createObjectNode(): couldn't " +
  ...
  public SearchCriteriaNode createPropertyNode( SearchCriteriaNode theParent, 
    ...
    */
    ...
    CriterionValue val = new CriterionValue( so, att, theParent.getId() + "." + propName );
  ...
  public SearchCriteria toSearchCriteria( SearchCriteriaNode node, String mergeValue )
    throws InvalidCriterionException
  {
	...
	beanName = objName;
      ...
      String scClassName = 
	COREUtilities.getSCPackageName( objName ) + 
	...
	if( "id".equals( cv.getPropertyName() ) &&
	  ...
	  addAttributeCriterion( criteria, cv, mergeValue, propNode.isMergeCriterionNode() );
	}
	...
	SearchCriteria subCriteria = toSearchCriteria( objNode, mergeValue );
	/*
	...
	*/
	...
	criteria.putCriterion( assocName, subCriteria );
      ...
      throw new InvalidCriterionException( "error creating SearchCriteria: " + ex.getMessage() );
    }
    ...
    throws InvalidCriterionException
  {
    ...
    if( isMergeCriterion && mergeValue != null ){
      MessageLog.printInfo( "QueryDesign.addAttributeCriterion(): mergeValue = " + mergeValue );
      vals.add( mergeValue );
    }else{
      //convert to simple Strings
      List vlps = cv.getValues();
    ...
    throws InvalidCriterionException
  {
    ...
    for( Iterator i = origVals.iterator(); i.hasNext(); ){
      ...
      try{
	if( att.getType().toString().equals( AttributeType.ALPHANUMERIC.toString() ) ){
	  //MessageLog.printInfo( "...ALPHANUMERIC" );
	  convertedVals.add( strVal );
	}else if( att.getType().toString().equals( AttributeType.NUMERIC.toString() ) ){
	  //MessageLog.printInfo( "...NUMERIC" );
	  convertedVals.add( new Long( strVal ) );
	}else if( att.getType().toString().equals( AttributeType.BOOLEAN.toString() ) ){
	  //MessageLog.printInfo( "...BOOLEAN" );
	  convertedVals.add( new Boolean( strVal ) );
	}else if( att.getType().toString().equals( AttributeType.DATE.toString() ) ){
	  //MessageLog.printInfo( "...DATE" );	  
	  convertedVals.add( DateFormat.getInstance().parse( strVal ) );
	}else if( att.getType().toString().equals( AttributeType.FLOAT.toString() ) ){
  ...
  public SearchCriteriaNode findNonEmptyChildSearchCriteriaNode( SearchCriteriaNode node ){
    SearchCriteriaNode theNode = null;
    if( node != null ){
      List propNodes = node.getPropertyNodes();
      for( Iterator i = propNodes.iterator(); i.hasNext(); ){
	...
	List objNodes = node.getObjectNodes();
	for( Iterator i = objNodes.iterator(); i.hasNext(); ){
	  SearchCriteriaNode aNode = 
	    findNonEmptyChildSearchCriteriaNode( (SearchCriteriaNode)i.next() );
	  if( aNode != null ){
	    theNode = aNode;
	    break;
	  }
  ...
  public Ontologable getOntologyRoot( String classname )
    throws Exception
  {
    /*
