...
public class CreateQueryForm extends ActionForm{
  ...
  public CreateQueryForm(){
	...
	if( so.getDisplayable() ){
	  _SOVLs.add( new ValueLabelPair( so.getClassname(), so.getLabel() ) );
	}
      }
    }catch( Exception ex ){
      MessageLog.printStackTrace( ex );
      throw new RuntimeException( "error getting searchable objects: " + ex.getMessage() );
    }
    ...
    throws ServletException
  {
	...
	design = new QueryDesign( label, label, classname );
      ...
      MessageLog.printStackTrace( ex );
      throw new ServletException( "error creating new query design: " + ex.getMessage() );
    }
    return design;
  }
	...
	if( nextStep != null ){
		...
		ActionError error = 
		    FormUtils.validateRequiredName( _label,
						    "createQueryForm.errors.queryLabelRequired",
						    new String[0] );
		if( error == null ){
		    error = FormUtils.validateUniqueName( _label, _queryDesigns.keySet(),
