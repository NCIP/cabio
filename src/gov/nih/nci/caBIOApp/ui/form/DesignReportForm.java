...
public class DesignReportForm extends ActionForm{
    ...
    throws ServletException
  {
	...
	MessageLog.printInfo( "DesignReportForm.updateReport(): movingLeft to colNum " + newColNum );
	if( newColNum > -1 ){
	...
	int newColNum = _selectedColSpec.getNewColumnNumber() + 1;
	MessageLog.printInfo( "DesignReportForm.updateReport(): movingRight to colNum " + newColNum +
			      ", and numCols == " + numCols );
	...
	throw new ServletException( "unknown operation: " + _updateOperationName );
      }
    }catch( Exception ex ){
