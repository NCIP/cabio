package gov.nih.nci.maservice.service.globus.resource;
...
import gov.nih.nci.maservice.common.MaGridServiceConstants;
import gov.nih.nci.maservice.stubs.MaGridServiceResourceProperties;
import gov.nih.nci.maservice.service.MaGridServiceConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class MaGridServiceResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(MaGridServiceResourceBase.class);

	private MaGridServiceResourceConfiguration configuration;
    ...
    public MaGridServiceResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.dataservice.DomainModel getDomainModel(){
		return ((MaGridServiceResourceProperties) getResourceBean()).getDomainModel();
	}
	
	public void setDomainModel(gov.nih.nci.cagrid.metadata.dataservice.DomainModel domainModel ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(MaGridServiceConstants.DOMAINMODEL);
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((MaGridServiceResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(MaGridServiceConstants.SERVICEMETADATA);
	...
	public MaGridServiceResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (MaGridServiceResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadDomainModelFromFile() {
      if(getDomainModel()==null){
		try {
			...
			((MaGridServiceResourceProperties) this.getResourceBean()).setDomainModel((gov.nih.nci.cagrid.metadata.dataservice.DomainModel) Utils.deserializeDocument(dataFile.getAbsolutePath(),
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((MaGridServiceResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
