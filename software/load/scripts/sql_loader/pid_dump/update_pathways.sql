...
insert into bio_pathways_tv(pathway_id, pathway_name, pathway_display, taxon, pathway_diagram_name, pathway_desc, pathway_diagram, big_id) select * from zstg_bio_pathways_tv;
commit;
...
update pid_participant p set p.PTM = (select distinct ptm from zstg_pid_comp_partcipant_ptm x where p.COMPLEX_ENTITY_ID = x.COMPLEX_ID and p.PID_COMPLEX_COMPONENT_ID = x.ORDER_OF_COMPLEX) where p.PTM is null and p.DISCRIMINATOR = 'ComplexComponent';
commit;

update pid_participant p set p.ACTIVITY_STATE = (select distinct x.ACTIVITY_STATE from zstg_pid_comp_partcipant_actst x where p.COMPLEX_ENTITY_ID = x.COMPLEX_ID and p.PID_COMPLEX_COMPONENT_ID = x.ORDER_OF_COMPLEX) where p.ACTIVITY_STATE is null and p.DISCRIMINATOR = 'ComplexComponent';
commit;

update pid_participant p set p.LOCATION = (select distinct x.LOCATION from zstg_pid_compl_participant_loc x where p.COMPLEX_ENTITY_ID = x.COMPLEX_COMPONENT_ID and p.PID_COMPLEX_COMPONENT_ID = x.ORDER_OF_COMPLEX) where p.LOCATION is null and p.DISCRIMINATOR = 'ComplexComponent';
commit;
...
update pid_participant p set p.PTM = (select distinct ptm from zstg_pid_fmly_prtpnt_ptm x where p.PID_FAMILY_ID = x.FAMILY_ID and p.PID_MEMBER_ID = x.MEMBER_ID) where p.PTM is null and p.DISCRIMINATOR = 'FamilyMember';
commit;
...
update pid_participant p set p.ACTIVITY_STATE = (select distinct x.ACTIVITY_STATE from zstg_pid_fmly_prtpnt_actst x where p.PID_FAMILY_ID = x.FAMILY_ID and p.PID_MEMBER_ID = x.MEMBER_ID) where p.ACTIVITY_STATE is null and p.DISCRIMINATOR = 'FamilyMember';
commit;

update pid_participant p set p.PID_INTERACTION_ID =
(select distinct interaction_id from zstg_pid_interactants 
...
insert into pid_interaction_evidence(interaction_id, evidence_id) select distinct i.id, e.id from pid_interaction i, evidence e, zstg_pid_interactionreference z where z.interaction_id = i.pid_interaction_id and z.pubmed_id=e.pubmed_id and to_char(e.sentence) = 'Used to publish Pathways';
commit;
