...
public class PagerBeanImpl implements PagerBean {
    ...
    public PagerItem[] getAvailableItems() throws Exception {
        ...
        if (_itemCount > 0) {
            if (_cachedItemsIndex[_startIdx] == null) {
                ...
                PagerItem[] newItems = _dataSource.getItems(_startIdx,
                    _displaySize);
                if (newItems == null) {
                    throw new Exception("newItems is null");
                }
                for (int i = 0; i < newItems.length; i++) {
                    PagerItemWrapper piw = new PagerItemWrapper(_startIdx + i,
                            newItems[i]);
                    items[i] = piw;
    ...
    public void scroll() throws Exception {
        if ("begin".equals(_scrollDirection)) {
            ...
            _startIdx = Math.min(_itemCount - 1, _startIdx
                    + _defaultDisplaySize);
            _displaySize = Math.min(_defaultDisplaySize, Math.min(_itemCount
                    - _startIdx, _defaultDisplaySize));
        }
