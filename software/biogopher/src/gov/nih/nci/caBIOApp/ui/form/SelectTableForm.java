...
import gov.nih.nci.caBIOApp.report.ExcelParser;
...
public class SelectTableForm extends ActionForm {
    ...
    public ActionErrors addTable() throws ServletException {
        ...
        if (_uploadedFile != null) {
            ...
            try {
                if (_worksheetNumber != null
                        && _worksheetNumber.trim().length() > 0) {
                    wrkSht = new Integer(Integer.parseInt(_worksheetNumber) - 1);
                }
                if (_headerRowNumber != null
                        && _headerRowNumber.trim().length() > 0) {
                    hdrRow = new Integer(Integer.parseInt(_headerRowNumber) - 1);
                }
                if (_dataStartRowNumber != null
                        && _dataStartRowNumber.trim().length() > 0) {
                    datRow = new Integer(
                            Integer.parseInt(_dataStartRowNumber) - 1);
                }
                MessageLog.printInfo("wrkSht = " + wrkSht + ", hdrRow = "
                        + hdrRow + ", datRow = " + datRow);
                ...
                errors.add("worksheetNumber", new ActionError(
                        "errors.outOfRange", ex.getMessage()));
                successful = false;
            }
            catch (IOException ex) {
                MessageLog.printStackTrace(ex);
                errors.add("uploadedFile", new ActionError(
                        "errors.invalidFormat", _uploadedFile.getFileName()));
                successful = false;
            }
            catch (Exception ex) {
                MessageLog.printStackTrace(ex);
            ...
            if (_selectedTable != null) {
                ...
                if (successful) {
                    ...
                    errors.add("worksheetNumber", new ActionError(
                            "errors.emptyfile", _worksheetNumber));
                }
            }
        }
        else {
    ...
    public void selectTable() throws ServletException {
    ...
    public ActionErrors validate(ActionMapping mapping,
        ...
        MessageLog.printInfo("validate: headerRowNumber = "
                + request.getParameter("headerRowNumber"));
        if (wrkSht != null && wrkSht.trim().length() > 0) {
            try {
                Integer.parseInt(wrkSht.trim());
            }
            catch (NumberFormatException ex) {
                errors.add("worksheetNumber", new ActionError("errors.nan",
                        wrkSht));
            }
        }
