...
public class CaBIOPagerDataSource implements PagerDataSource {
    ...
    public CaBIOPagerDataSource(String classname, String beanId,
            String roleName, String eventHandler) {
        _eventHandler = eventHandler;
        SODUtils sod = SODUtils.getInstance();
        _searchableObject = sod.getSearchableObject(classname);
        if (_searchableObject == null) {
            throw new MissingResourceException("Cannot find SearchableObject",
                    classname, classname);
        }
        Association assoc = sod.getAssociationWithRole(_searchableObject,
            roleName);
        if (assoc == null) {
            throw new RuntimeException(
                    "Couldn't find association with roleNam " + roleName
                            + " on " + classname);
        }
        try {
            ...
            String recipRole = CaBIOUtils.getReciprocalRole(thisCrit.getBeanName()
                    + "." + roleName);
            ...
            throw new RuntimeException("Error initializing datasource: "
                    + ex.getMessage());
        }
    }
    ...
    private void init(String classname, SearchCriteria criteria) {
        ...
        try {
            ...
            if (ex instanceof ClassNotFoundException
                ...
                throw new RuntimeException("Error setting up bean: "
                        + ex.getMessage());
            }
        }
        MessageLog.printInfo("CaBIOPagerDataSource.init(): _criteria is "
                + _criteria.getClass().getName());
            ...
            if (att == null) {
                throw new RuntimeException(
                        "CaBIOPagerDataSource.init(): couldn't find attribute "
                                + propName + " on "
                                + _searchableObject.getClassname());
            }
        ...
        if (_searchResult == null) {
            ...
            for (int i = 0; i < numItems; i++) {
                ...
                for (Iterator it = labelProps.iterator(); it.hasNext(); j++) {
                    ...
                    try {
                        m = objects[i].getClass().getMethod(methodName,
                            new Class[0]);
                    }
                    catch (Exception ex) {
                        try {
                            ...
                            throw new Exception("No access to method '"
                                    + methodName + "' on "
                                    + objects[i].getClass().getName() + ": "
                                    + ex.getMessage());
                        }
                ...
                for (Iterator it = assocs.iterator(); it.hasNext(); j++) {
                    Association assoc = (Association) it.next();
                    vals[j] = "<a href=\"javascript:" + _eventHandler + "('"
                            + objects[i].getClass().getName() + "', '" + id
                            + "', '" + assoc.getRole() + "')\">"
                            + assoc.getLabel()
                            + getCountString(objects[i], assoc.getRole())
                            + "</a>";
                }
    ...
    private void initializeItems() throws Exception {
        ...
        if (_itemCount < 0) {
            _itemCount = 0;
            MessageLog.printInfo("CaBIOPagerDataSource.initializeItems(): _itemCount = "
                    + _itemCount);
        }
        _initialized = true;
    }
