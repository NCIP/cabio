...
my $dbh = DBI->connect($ENV{'DBI_DRIVER'}, $ENV{'SCHEMA'}, $ENV{'SCHEMA_PWD'}) || die  "Error ".DBI->errstr;
my $sql = qq(
select distinct c.taxon_ID, c.gene_ID, c.SYMBOL, c.chromosome_ID, a.CHR_START, a.CHR_STOP,b.UNIGENE_CLUSTER,a.FEATURE_TYPE, a.GROUP_LABEL from zstg_seqgene a, zstg_gene2unigene b, gene_tv c   where substr(a.FEATURE_ID, instr(a.FEATURE_ID,':')+1) = to_char(b.GENEID) and substr(b.UNIGENE_CLUSTER,instr(b.UNIGENE_CLUSTER,'.')+1)= c.CLUSTER_ID
);
my $sth = $dbh->prepare($sql);
$sth->execute();
...
my %markerHash;
my $dbh = DBI->connect($ENV{'DBI_DRIVER'}, $ENV{'SCHEMA'}, $ENV{'SCHEMA_PWD'}) || die  "Error ".DBI->errstr;
my $sql = qq(select distinct c.ID, d.taxon_ID, d.chromosome_ID, c.marker_ID, a.CHR_START, a.CHR_STOP, a.FEATURE_TYPE, a.GROUP_LABEL from zstg_seqsts a, marker c, chromosome d where substr(a.FEATURE_ID, instr(a.FEATURE_ID,':')+1) = to_char(c.MARKER_ID) and decode(instr(a.chromosome,'_'),0,a.chromosome,substr(a.chromosome,0,instr(a.chromosome,'_')-1)) = d.chromosome_number and a.TAX_ID = d.TAXON_ID and c.TYPE = 'UNISTS');
my $sth = $dbh->prepare($sql);
$sth->execute();
...
my %ESTHash;
my $dbh = DBI->connect($ENV{'DBI_DRIVER'}, $ENV{'SCHEMA'}, $ENV{'SCHEMA_PWD'}) || die  "Error ".DBI->errstr;
my $sql = qq(SELECT DISTINCT QNAME, chromosome_ID, TSTART, TEND, 'EST' as FEATURE_TYPE from zstg_human_est where CHROMOSOME_ID is NOT NULL);
my $sth = $dbh->prepare($sql);
$sth->execute();
...
my $sql = qq(SELECT DISTINCT QNAME, chromosome_ID, TSTART, TEND, 'EST' as FEATURE_TYPE from zstg_mouse_est where CHROMOSOME_ID is NOT NULL);
my $sth = $dbh->prepare($sql);
$sth->execute();
$sth->bind_columns(\$accNo,\$chrId, \$start,\$stop, \$ftype);
while($sth->fetch()) {
...
my %MRNAHash;
my $dbh = DBI->connect($ENV{'DBI_DRIVER'}, $ENV{'SCHEMA'}, $ENV{'SCHEMA_PWD'}) || die  "Error ".DBI->errstr;
my $sql = qq(SELECT DISTINCT QNAME, chromosome_ID, TSTART, TEND, 'MRNA' as FEATURE_TYPE from zstg_human_mrna where CHROMOSOME_ID is NOT NULL);
my $sth = $dbh->prepare($sql);
$sth->execute();
$sth->bind_columns(\$accNo,\$chrId, \$start,\$stop, \$ftype);
while($sth->fetch()) {
...
my $sql = qq(SELECT DISTINCT QNAME, chromosome_ID, TSTART, TEND, 'MRNA' as FEATURE_TYPE from zstg_mouse_mrna where CHROMOSOME_ID is NOT NULL);
my $sth = $dbh->prepare($sql);
$sth->execute();
$sth->bind_columns(\$accNo,\$chrId, \$start,\$stop, \$ftype);
while($sth->fetch()) {
